...
LEFT JOIN master_levels ON(tran_outcomes.level_id=master_levels.id) 
LEFT JOIN master_biz_impact ON(tran_outcomes.business_impact_id=master_biz_impact.id) 
WHERE tran_outcomes.company_id=".$this->db->escape($company_id)." AND master_levels.id=".$this->db->escape($level_id)."  GROUP BY tran_outcomes.id) AS B ON(A.id=B.id AND A.start_date=B.start_date) LEFT JOIN spider_chart_data AS C ON (A.id=C.outcome_id AND C.company_id=".$this->db->escape($company_id)." )  WHERE  C.data_date IS NOT NULL  ORDER BY A.id";
//echo $sql;exit;
		 $query = $this->db->query($sql);
		 return $query->result_array();
	...
	function select_all_outcomeList($id='',$company_id){
		if($id){
		 ...
		 $sql = " SELECT tran_outcomes.`id`,tran_outcomes.`company_id`,
                ...
                master_biz_impact.business_impact,master_stages.stage,master_levels.level,master_color.name as color,master_status.name,tran_status.start_date,tran_status.status_id,tran_color.class_id,(CASE  WHEN tran_outcomes.`status` = 0 THEN 'Active' ELSE 'In-Active' END)  as outcome_status
